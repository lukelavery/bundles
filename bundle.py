...
class Bundle:
    ...
    def __init__(
        ...
        self.data = self.get_bundle_data(bundle_path=bundle_path)
        self.name = f'{date.today().year}.{date.today().month}.{date.today().day} {os.path.basename(bundle_path)}.pdf'
        self.documents = Documents(self)
        self.paths = {}

        index_path = os.path.join(bundle_path + '/index_template.docx')
        if os.path.isfile(index_path):
            self.index = Index(bundle=self, index_path=index_path)
        else:
            self.index = Index(bundle=self, index_path='assets/index.docx')

    def get_bundle_data(self, bundle_path: str):
        ...
        for directory in os.listdir(bundle_path):
            entry_list = []
            sub_dir = os.path.join(bundle_path, directory)
